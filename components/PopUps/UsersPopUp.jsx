...
const UsersPopUp = (props) => {
  ...
  const selectUserHandler = async (user) => {
        try {
          const combinedChatId =  currentUser.uid > user.uid ? currentUser.uid + user.uid : user.uid + currentUser.uid;
        ...
        <div className="absolute w-full">
          {users &&
            Object.values(users).map((user) => (
              <div onClick={()=> selectUserHandler(user)} className="flex items-center gap-4 rounded-full hover:bg-c5 py-2 px-4 cursor-pointer">
